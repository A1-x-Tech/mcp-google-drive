...
export class GoogleDriveClient {
  ...
  private async accessToken(forceRefresh = false): Promise<string> {
    if (!this.canRefresh()) {
      ...
      // GOOGLE_DRIVE_ACCESS_TOKEN keeps behaving exactly as before.
...
/**
 * One cheap Google Drive read, used to verify a fresh in-chat login against the
 ...
 * finished. Throws GoogleDriveError exactly like the client does, so the caller
...
export async function probeApi(accessToken: string): Promise<void> {
  const base = (process.env.GOOGLE_DRIVE_API_BASE || DEFAULT_BASE).replace(/\/+$/, "");
  const res = await fetch(`${base}/drive/v3/about?fields=user`, {
