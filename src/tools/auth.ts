 ...
 * docs/TOOLS.md). The component always adds the identity scopes on top. The full `drive` scope is requested because the tool surface includes sharing, trash, deletion and comments on files this server did not create; `drive.file` would silently fail on anything the user opened elsewhere.
 */
...
/**
 * `verifyIdentity` for the component: one cheap Google Drive read, plus the
 ...
 * is the point: OIDC answers even when Google Drive API is switched off, so a
...
async function verifyIdentity(accessToken: string): Promise<{ email?: string }> {
  try {
    ...
    if (error instanceof GoogleDriveError && error.status === 403 && isApiDisabled(error.body)) {
      throw new OAuthError("accessNotConfigured", "Google Drive API", error.status);
    }
    throw error;
  }
...
/**
 * The single source of the auth wiring: serverName "drive" puts the stored
 * login in ~/.config/mcp-google-drive/credentials.json (the component
 * prefixes "mcp-google-"), envPrefix keeps the provider reading the exact
 * GOOGLE_DRIVE_* variables config.ts documents — which is what preserves the
