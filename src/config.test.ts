...
/**
 * Missing credentials must never throw here: throwing would kill the process
...
test("no credentials at all is not an error — the config loads with empty fields", () => {
  withEnv({}, () => {
    ...
    assert.equal(config.apiBase, "https://www.googleapis.com");
    assert.equal(hasCredentials(config), false);
  });
});

test("a partial OAuth triple reports incomplete_oauth_config", () => {
  assert.equal(reasonOf({ GOOGLE_DRIVE_CLIENT_ID: "id" }), "incomplete_oauth_config");
  assert.equal(
    reasonOf({ GOOGLE_DRIVE_CLIENT_ID: "id", GOOGLE_DRIVE_CLIENT_SECRET: "secret" }),
  ...
  assert.equal(
    reasonOf({ GOOGLE_DRIVE_REFRESH_TOKEN: "rt", GOOGLE_DRIVE_ACCESS_TOKEN: "at" }),
    "incomplete_oauth_config",
  );
});
...
test("a static access token alone is enough", () => {
  withEnv({ GOOGLE_DRIVE_ACCESS_TOKEN: "at" }, () => {
    const config = loadConfig();
  ...
  withEnv(
    {
      ...
      GOOGLE_DRIVE_TIMEOUT_MS: "not-a-number",
...
test("numeric overrides are honored when valid", () => {
  withEnv(
    { GOOGLE_DRIVE_ACCESS_TOKEN: "at", GOOGLE_DRIVE_TIMEOUT_MS: "1000", GOOGLE_DRIVE_MAX_RETRIES: "0" },
    () => {
      const config = loadConfig();
...
test("the API base override is honored", () => {
  withEnv({ GOOGLE_DRIVE_ACCESS_TOKEN: "at", GOOGLE_DRIVE_API_BASE: "https://example.test" }, () => {
